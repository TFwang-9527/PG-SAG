	...
	const bool debug)
{
  ...
  torch::Tensor load = torch::full({H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		W, H,
		multi_mask.contiguous().data<float>(),
		...
		load.contiguous().data<float>(),
		render_geo,
		debug);
  }
  return std::make_tuple(rendered, out_color, radii, out_observe, out_all_map, out_plane_depth,load, geomBuffer, binningBuffer, imgBuffer);
}
	...
	const bool debug) 
{
  ...
  {  
	  ...
	  dL_dloads.contiguous().data<float>(),
